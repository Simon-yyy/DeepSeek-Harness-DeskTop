...
// Handle image paste: save to local temp file and return absolute path
...
//   node  -> DSH_NODE env override, else scan well-known node install
//            locations (including non-PATH ones like D:\hclaw\node), else
//            `node` on PATH, else Electron's embedded Node as last resort.
//            Scanning real paths matters because shortcuts launched from the
//            desktop do not inherit a terminal's PATH customizations.
//   dsh   -> DSH_BIN env (a bin.js path), else scan the npx cache for the
//            newest @deepseek-ai/dsh, else `dsh` on PATH.
// ---------------------------------------------------------------------------
function resolveNode() {
  if (process.env.DSH_NODE && fs.existsSync(process.env.DSH_NODE)) return process.env.DSH_NODE;
  // Well-known install roots, including non-standard ones seen on this machine.
  const roots = [
    "D:\\hclaw\\node",
  ...
  // Last resort: Electron's own embedded Node (v20 — may not satisfy dsh's
  // ^22.19 || >=24 requirement, so it is only a fallback).
  return process.execPath;
}

function resolveDshBin() {
  if (process.env.DSH_BIN && fs.existsSync(process.env.DSH_BIN)) return process.env.DSH_BIN;
  // Scan the npx cache for the newest installed copy of @deepseek-ai/dsh.
  // Check several likely cache roots so the app survives npm cache moves.
  const cacheRoots = [
...
// ---------------------------------------------------------------------------
// Backend lifecycle: reuse an already-running dsh web on WEB_PORT; otherwise
// spawn one and track it so we can stop it when the last window closes.
// ---------------------------------------------------------------------------
...
function spawnBackend() {
  ...
  if (dshBin && /\.cmd$/i.test(dshBin)) {
    // dsh.cmd shim on PATH
    child = spawn(dshBin, ["--profile", "web"], { cwd, env, stdio: "ignore", windowsHide: true, shell: true });
  } else if (dshBin) {
    // dsh bin.js — run it under node. If the resolved node is Electron's own
    // binary, ELECTRON_RUN_AS_NODE=1 switches it into plain Node mode.
    const runEnv = { ...env };
...
function createWindow() {
  ...
  const appIcon = nativeImage.createFromPath(iconCandidate);
  ...
  // Explicitly enforce taskbar and window icon
  mainWindow.setIcon(appIcon);

  // Prevent web backend from overriding the desktop window title
  ...
  // Test hook: DSH_DESKTOP_TEST=1 — write a marker once the page finishes
  // loading, then quit. Lets automation verify the window actually reached
  // the web GUI without needing to see the screen.
  if (process.env.DSH_DESKTOP_TEST === "1") {
    ...
    mainWindow.webContents.on("console-message", (_e, level, message) => { log(`console[${level}]: ${message.slice(0, 200)}`); });
  }

  // Open external links in the system browser, keep app-internal navigation in-window.
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    try {
      ...
      // invalid url -> delegate to system shell safely
    }
