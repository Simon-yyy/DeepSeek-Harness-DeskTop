...
- 🛡️ **双轨免死认证与原子幂等导航**：主进程自动签发官方 HMAC-SHA256 签名鉴权 Cookie 注入 Session，配合激活 URL Token 实现双保险认证；由原子幂等导航控制器（`safeNavigateToWorkbench`）串行互斥调度，彻底终结 Chromium `net::ERR_ABORTED (-3)` 与卡启动屏问题。
...
- 🌐 **全局网络免拦截垫片与看门狗**：底层通过 `NODE_OPTIONS` 首毫秒注入 `network-shim.js`，为第三方 AI 服务商请求注入 `cline/3.0.0` 白名单 User-Agent，深度保护 Node 原生 `fetch` 标头与 `Authorization` 继承；集成操作系统级管道断管自毁看门狗，父进程闪退或退出时子进程毫秒级自尽，彻底根除孤儿进程。
...
#### Q4：调用某些第三方 AI 中转站时报 403 或 User-Agent 拦截？
- **答**：**已原生解决**。
  - 客户端内置的 `network-shim.js` 网络兼容垫片会在 Node 子进程启动首毫秒挂载；
  - 自动识别第三方 AI 推理请求并无感注入 `cline/3.0.0` 白名单 UA，同时深度保护 `Authorization` 认证标头继承，杜绝中转站 WAF 误杀。
