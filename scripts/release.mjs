  ...
  if (newVersion !== oldVersion) {
    ...
    if (fs.existsSync(lockPath)) {
      try {
        ...
        log(`✓ Updated package-lock.json version to ${newVersion}`);
      } catch (_e) {}
    }
  }

  // 1.5 Prune bundled-backend to accelerate installation
  log(`\n🧹 Pruning bundled-backend before build...`);
  const pruneScript = path.join(rootDir, 'scripts', 'prune-backend.mjs');
  if (fs.existsSync(pruneScript)) {
    execSync(`"${process.execPath}" "${pruneScript}"`, { stdio: 'inherit', cwd: rootDir });
  }
